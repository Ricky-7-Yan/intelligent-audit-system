...
<body>
    ...
    <main class="main">
      ...
      <section class="command-panel">
        <div class="panel">
          ...
          <div class="toolbar mt-16"><button class="btn primary" id="runAudit">{ent('运行 Agent 审计')}</button><button class="btn" id="loadControls">{ent('查看控制库')}</button><a class="btn" id="downloadReport" href="#" target="_blank">{ent('下载报告')}</a><a class="btn" id="downloadDelivery" href="#" target="_blank">{ent('下载交付包')}</a></div>
      ...
      <section class="panel mt-16"><h2 class="panel-title">{ent('审计交付包预览')}</h2><div id="deliveryPreview" class="delivery-grid"><p class="muted">{ent('选择或运行审计档案后展示底稿、证据、访谈和现场工作日程。')}</p></div></section>
  ...
  <script>
    ...
    async function loadTemplates() {{ try {{ const data = await apiFetch("/api/audit/templates"); const grid = qs("#scenarioGrid"); clearNode(grid); data.templates.forEach((tpl) => grid.appendChild(el("div", {{ class: "card scenario", "data-item": tpl.name, "data-type": tpl.audit_type, "data-standard": tpl.standard, "data-risk": tpl.risk_level }}, [el("strong", {{ text: tpl.name }}), el("p", {{ class: "muted", text: `范围：${{(tpl.scope || []).slice(0, 3).join("、")}}` }}), el("div", {{ class: "muted", text: `交付物：${{(tpl.deliverables || []).slice(0, 2).join("、")}}` }})]))); bindScenarios(); }} catch {{ bindScenarios(); }} }}
    loadTemplates();
    loadRuns();
  </script>
</body>
